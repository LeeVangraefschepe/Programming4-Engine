...
struct MatchMakingKeyValuePair_t
{
	...
	{
		strncpy_s(m_szKey, sizeof(m_szKey), pchKey, _TRUNCATE);
		strncpy_s(m_szValue, sizeof(m_szValue), pchValue, _TRUNCATE);
	}
	char m_szKey[ 256 ];
	char m_szValue[ 256 ];
};
...
inline const char* servernetadr_t::ToString(uint32 unIP, uint16 usPort) const
{
	...
	_snprintf_s(s[nBuf], sizeof(s[nBuf]), _TRUNCATE, "%u.%u.%u.%u:%i", (int)(ipByte[0]), (int)(ipByte[1]), (int)(ipByte[2]), (int)(ipByte[3]), usPort);
#else
	_snprintf_s(s[nBuf], sizeof(s[nBuf]), _TRUNCATE, "%u.%u.%u.%u:%i", (int)(ipByte[3]), (int)(ipByte[2]), (int)(ipByte[1]), (int)(ipByte[0]), usPort);
#endif
...
{
	strncpy_s(m_szServerName, sizeof(m_szServerName), pName, _TRUNCATE);
}
